...
def mock_tracking(seq,dt,row_cnt):
    faker = Faker(locale='zh_CN')
    faker.seed(int(random.random() * 10000000000))
    random.seed(int(random.random() * 10000))
    print ('step into tracking.........')
    with open('/home/dataservice/workspace/mock/result/seq/tracking/test_tracking_%s.csv' % dt, 'w') as f:
    	for num in range(1, row_cnt + 1):
                ...
                f.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (seq,mac,mac_md5,tracking_url,uri_extension,media_type,device_type,campaign,province,city,dt))
                seq += 1
                if num % 10000 == 0:
                    f.flush()
if __name__ == "__main__":
    start_dt = datetime.datetime(2018, 11, 01)
    ...
    for index, i in enumerate(range(0, total, page_size)):
        ...
        p.apply_async(mock_tracking,(i + 1,t_str, page_size))
        print("mock_tracking, params:%s,%s,%s" % (t_str, i + 1, page_size))
    p.close()
    p.join()
    print("finish")
